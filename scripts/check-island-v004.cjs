...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(root, 'artifacts/island-v004-studio.png') });
    await canvas.screenshot({ path: path.join(root, 'artifacts/island-v004-three.png') });
    for (const name of ['浅凹印花一体底盘', '穆夏陶瓷铺装']) {
      await page.getByRole('button', { name: `独显${name}`, exact: true }).click();
      await page.getByRole('button', { name: `退出独显${name}`, exact: true }).waitFor();
      await canvas.screenshot({ path: path.join(root, `artifacts/island-v004-${name === '浅凹印花一体底盘' ? 'body' : 'paving'}.png`) });
    ...
    assert.equal(record.modelUrl, '/assets/island/v004/model.glb');
    ...
    const report = JSON.parse(await fs.readFile(path.join(root, 'public/assets/island/v004/geometry-report.json')));
    ...
    const result = { modelVersion: 'v004', embeddedFloorImage: true, semanticParts: record.parts.length, isolateAndReset: true, closedSolidParts: true, errors };
    await fs.writeFile(path.join(root, 'artifacts/island-v004-check.json'), JSON.stringify(result, null, 2));
