...
(async () => {
  ...
  try {
    ...
    await canvas.screenshot({ path: path.join(root, 'artifacts/island-v006-three.png'), timeout: 90000 });
    for (const name of ['紧凑直背凸起底盘', '穆夏陶瓷铺装']) {
      ...
      await canvas.screenshot({ path: path.join(root, `artifacts/island-v006-${name === '紧凑直背凸起底盘' ? 'body' : 'paving'}.png`), timeout: 90000 });
    ...
    assert.equal(record.modelUrl, '/assets/island/v006/model.glb');
    ...
    const result = { modelVersion: 'v006', embeddedTextures: 2, semanticParts: record.parts.length, isolateAndReset: true, errors };
    await fs.writeFile(path.join(root, 'artifacts/island-v006-check.json'), JSON.stringify(result, null, 2));
