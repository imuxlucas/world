...
(async()=>{
 ...
 try{
  for(const [id,version] of [['island','v009'],['craft','v003'],['carousel','v004']]){
   ...
   await canvas.screenshot({path:path.join(root,`artifacts/round-two-${id}.png`),timeout:120000});
  ...
  assert.deepEqual(errors,[]);await fs.writeFile(path.join(root,'artifacts/round-two-check.json'),JSON.stringify({report,errors},null,2));console.log({report,errors});
