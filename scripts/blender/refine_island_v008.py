...
ROOT=b.ROOT
OUT=ROOT/'public/assets/island/v008'
SOURCE=ROOT/'asset-sources/island/v008'
R=5.2
TOP=.56
DEPTH=.052
# Dense, evenly resolved front surface; the rear needs fewer columns.
ANGLES=sorted(set([-math.pi+math.tau*i/256 for i in range(256)]+[-.53+1.06*i/640 for i in range(641)]))
ANGULAR=len(ANGLES)
...
def radial_bump(r):
    """0.24m strip with a near-vertical rear wall and tiny upper/lower fillets."""
    if r<=4.960:return 0.0
    if r<4.968:return .04*smoother((r-4.960)/.008)
    if r<4.974:return .04+.92*smoother((r-4.968)/.006)
    if r<4.982:return .96+.04*smoother((r-4.974)/.008)
    return 1.0
...
def recess_radius(a,z):
    ...
    vertical=min(lower,upper)
    # A circular junction, tangent to both edges, replaces the square min().
    corner=.16
    distance=min(side,vertical)
    if side<corner and vertical<corner:
        distance=corner-math.hypot(corner-side,corner-vertical)
    return R-DEPTH*smoother(distance/.045)
...
def make_body(parent):
    ...
    # A 0.24m-deep raised strip; most of the height drops across only 6mm.
    radii=[5.20,5.19,5.17,5.14,5.11,5.08,5.02,4.982,4.974,4.968,4.960,4.93,.02]
    ...
    art=b.material('Glitter inscription / glazed ceramic print',(1,1,1),rough=.24,metal=.12)
    shader=art.node_tree.nodes.get('Principled BSDF');shader.inputs['Coat Weight'].default_value=.45
    ...
    # Fit the visible outline, excluding the PNG's large blank margins.
    # Actual colored bounds: x=13..2158, y=128..586 in a 2172x724 image.
    image_width=4.34*2172/2145
    image_height=.53*724/458
    center_z=.475
    for face in obj.data.polygons:
        ...
        front=sum(p.y for p in points)<0
        if 2<=ring<99 and front and abs(x)<2.46:
            face.material_index=1
        for li in face.loop_indices:
            p=obj.data.vertices[obj.data.loops[li].vertex_index].co
            uv.data[li].uv=(1085.5/2172+p.x/image_width,1-357/724+(p.z-center_z)/image_height)
    return obj

def make_paving(parent,mat):
    radii=[.02,1.0,2.0,3.0,4.0,4.70,4.93]
    verts=[]
    for r in radii:
        for a in ANGLES:verts.append((r*math.sin(a),-r*math.cos(a),TOP+.006))
    faces=[]
    ...
    for face in obj.data.polygons:
        for li in face.loop_indices:
            p=obj.data.vertices[obj.data.loops[li].vertex_index].co
            uv.data[li].uv=(p.x/(4.935*2/.98)+.5,p.y/(4.935*2/.98)+.5)
    return obj
...
def main():
    ...
    base=b.part('island_base','紧凑直背凸起底盘','凸起进深 0.24m；平面铺装与后缘间隔 0.03m，背面直落并保留小倒角。')
    border=b.part('island_border','四边清晰浅凹陶瓷边缘','凹面四角 0.16m 连续圆角，去描边闪粉铭文，保留瓷白留边。')
    make_body(base)
    ...
    record.update(subtitle='圆角凹面 · 闪粉铭文 v008',description='保留梯形上沿，凹面四角改为连续圆角；铭文去描边，蓝粉花纹与文字增加细碎闪粉质感。',
        notes=['凸起进深 0.24m，铺装与凸起后缘相隔 3cm。','凹面四角采用 0.16m 圆角，保留四边统一收边。','imagegen 编辑无描边闪粉贴图，搭配低粗糙度清漆材质。'],
        modelUrl='/assets/island/v008/model.glb',thumbnail='/assets/island/v008/thumbnail.png',source={'label':'原创 Blender 圆角闪粉底盘 · v008','path':'asset-sources/island/v008/source.blend'})
    b.consolidate();stats,report=b.geometry_report('island')
    report['construction']={'analyticRoundedTrapezoid':True,'pixelDrivenGeometry':False,'rimInwardDepth':.24,'flatPlaneStartsAtRadius':4.96,'pavingRadius':4.93,'rearFace':'near-vertical with 8mm upper/lower fillets','recessDepth':DEPTH,'horizontalRecessFillet':.045,'verticalRecessFillet':.045,'visibleArtworkWidth':4.34,'visibleArtworkHeight':.53}
    ...
    print('ISLAND_V008_READY',json.dumps(stats),flush=True)
