...
ROOT=b.ROOT
OUT=ROOT/'public/assets/island/v007'
SOURCE=ROOT/'asset-sources/island/v007'
R=5.2
TOP=.56
DEPTH=.052
...
def recess_radius(a,z):
    ...
    side_factor=smoother(side/.045)
    vertical_factor=smoother(min(lower,upper)/.045)
...
def main():
    ...
    border=b.part('island_border','四边清晰浅凹陶瓷边缘','四边统一 45mm 收边、52mm 凹深，按彩色图案实际边界留白。')
    make_body(base)
    ...
    record.update(subtitle='0.24m 窄边凸起 · 四周瓷白留边 v007',description='平整铺装与凸起分开；四边统一收边截面，彩色铭文横向放宽、纵向缩短并完整留边。',
        notes=['凸起进深 0.24m，完整铺装半径微缩至 4.93m，与凸起后缘相隔 3cm。','凹槽四边统一 45mm 收边、52mm 凹深；前缘网格加密。','使用原始批准贴图，按实际彩色边界映射至 4.34m × 0.53m。'],
        modelUrl='/assets/island/v007/model.glb',thumbnail='/assets/island/v007/thumbnail.png',source={'label':'原创 Blender 窄边凸起底盘 · v007','path':'asset-sources/island/v007/source.blend'})
    ...
    print('ISLAND_V007_READY',json.dumps(stats),flush=True)
