...
(async () => {
  ...
  try {
    ...
    await canvas.screenshot({ path: path.join(root, 'artifacts/island-v005-three.png'), timeout: 90000 });
    for (const name of ['加厚平滑凸起底盘', '穆夏陶瓷铺装']) {
      await page.getByRole('button', { name: `独显${name}`, exact: true }).click();
      await page.getByRole('button', { name: `退出独显${name}`, exact: true }).waitFor();
      await canvas.screenshot({ path: path.join(root, `artifacts/island-v005-${name === '加厚平滑凸起底盘' ? 'body' : 'paving'}.png`), timeout: 90000 });
    ...
    assert.equal(record.modelUrl, '/assets/island/v005/model.glb');
    ...
    const result = { modelVersion: 'v005', embeddedTextures: 2, semanticParts: record.parts.length, isolateAndReset: true, errors };
    await fs.writeFile(path.join(root, 'artifacts/island-v005-check.json'), JSON.stringify(result, null, 2));
