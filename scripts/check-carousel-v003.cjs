...
(async()=>{
 ...
 try{
  ...
  await canvas.screenshot({path:path.join(root,'artifacts/carousel-v003-three.png'),timeout:90000});
  for(const label of ['圆形底座','三匹雕刻木马','伞顶与花檐']){
   await page.getByRole('button',{name:`独显${label}`,exact:true}).click();
   await canvas.screenshot({path:path.join(root,`artifacts/carousel-v003-${label}.png`),timeout:90000});
  ...
  assert.equal(asset.modelUrl,'/assets/carousel/v003/model.glb');
  ...
  assert.equal(gltf.nodes.filter(n=>n.name.startsWith('Undistorted raised fleur de lis')).length,16);
  assert.equal(gltf.nodes.filter(n=>n.name.startsWith('Valance colored bulb')).length,128);
  assert.equal(gltf.nodes.filter(n=>n.name.startsWith('Small rail pearl')).length,96);
  assert.ok(gltf.images.length>=4 && gltf.images.every(i=>i.bufferView!==undefined));
  assert.deepEqual(errors,[]);
  const report={version:'carousel-v003',parts:asset.parts.length,embeddedImages:gltf.images.length,reliefFlowers:16,coloredBulbs:128,smallPearls:96,partIsolation:true,errors};
  await fs.writeFile(path.join(root,'artifacts/carousel-v003-check.json'),JSON.stringify(report,null,2));console.log(report);
