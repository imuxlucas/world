...
export const OKR_EDITORIAL: Record<ObjectiveId, { title: string; action: string; acceptance: string }[]> = {
  ...
  o2: [
    ...
    { title: '用可运行的方案，推动智能交互按预期落地。', action: '运用 O1 方法，以概念方案和动态 Demo 比较解法、说明取舍；按需交付设计稿、Demo、组件或代码，明确状态与动效并跟进验收。', acceptance: '主体库、词槽、智能交互提案及元素过渡、抽屉动效案例，保留方案比较、落地效果与问题解决记录。' },
    { title: '把核心能力带到移动端，做出经使用验证的体验亮点。', action: '结合移动端输入、空间与情境优化核心链路，在做同款、局部编辑中探索 GUI 与 LUI 协同，形成 1—2 个经实际使用验证的亮点。', acceptance: 'APP 核心链路迭代与 GUI × LUI 场景案例；以任务表现和使用反馈说明价值，并据此优化。' },
  ],
  o3: [
    ...
    { title: '沉淀 5 个 AI 交互组件，让设计持续产生价值。', action: '与超超、Minna 维护 tad-universal，从真实业务提炼 5 个智能交互组件，补齐场景、状态、异常、设计依据与示例，并完成业务应用。', acceptance: '5 个 AI 组件、使用规范与应用案例；以业务可用性和复用价值验收，结合创新点沉淀具备申报价值的专利提案。' },
