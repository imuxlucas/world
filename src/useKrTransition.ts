...
import type { Objective } from './okrContent';
...
export function useKrTransition(objective: Objective, kr: number, open: boolean) {
  const [displayed, setDisplayed] = useState({ objective, kr });
  ...
  useLayoutEffect(() => {
    if (!open || reducedMotion || displayed.objective.id !== objective.id) {
      if (displayed.objective !== objective || displayed.kr !== kr) setDisplayed({ objective, kr });
      setPhase('idle');
      return;
    }
    if (displayed.kr === kr) {
    ...
    const timer = window.setTimeout(() => {
      setDisplayed({ objective, kr });
      setPhase('in');
    }, KR_EXIT_MS);
    // A newer navigation always wins; never replay stale tab content.
    return () => window.clearTimeout(timer);
  }, [objective, kr, open, reducedMotion, displayed.objective, displayed.kr]);
